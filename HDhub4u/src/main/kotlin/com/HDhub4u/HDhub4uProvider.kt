...
class HDhub4uProvider : MainAPI() {
    ...
    //     override var mainUrl: String = runBlocking { HDhub4uPlugin.getDomains()?.HDHUB4u ?: "..." }
    // That's a *blocking* network call fired at object-construction time
    // (i.e. during plugin registration in HDhub4uPlugin.load()), which can
    // stall whatever thread calls registerMainAPI(). It only runs once per
    // process for this class (unlike HubCloud, which got constructed
    // repeatedly), but it's still an unnecessary synchronous network hop
    // during app startup, and if the raw.githubusercontent.com fetch is
    // slow/unreachable it directly delays plugin load.
    //
    // Fix: start with a safe static fallback (no network call), then
    // refresh it opportunistically the first time a suspend function that
    // actually needs it runs (getMainPage/search/load), using the cached
    // domain list from HDhub4uPlugin. If the fetch fails, we silently keep
    // the fallback — no crash, no blocking.
    // ------------------------------------------------------------------
    override var mainUrl: String = "https://hdhub4u.glass"
    ...
    /** Refreshes mainUrl from the remote domain list without ever blocking a thread. */
    private suspend fun refreshDomain() {
        runCatching {
            HDhub4uPlugin.getDomains()?.HDHUB4u
        }.getOrNull()?.takeIf { it.isNotBlank() }?.let { mainUrl = it }
    }
    ...
    // FIX: renamed target type from "Document" to "SearchDocument" (see Utils.kt) to
    // remove the accidental name collision with org.jsoup.nodes.Document.
    private fun SearchDocument.toSearchResult(): SearchResponse {
        return newMovieSearchResponse(
            name = postTitle.orEmpty(),
            url = permalink.orEmpty(),
            type = TvType.Movie
        ) {
    ...
    override suspend fun search(query: String, page: Int): SearchResponseList {
        ...
        val response = try {
            ...
            Log.e("HDhub4u-Search", "Search request failed: ${e.message}")
            null
        }

        // FIX: was `response?.hits!!.map { ... }` — a null response (network error,
        // malformed JSON, or empty results) would throw a raw NullPointerException
        // straight out of search(). Now degrades to an empty list instead.
        val hits = response?.hits ?: emptyList()
        return hits.mapNotNull { it.document }
    ...
    override suspend fun load(url: String): LoadResponse {
        refreshDomain()
        val doc = app.get(url, cacheTime = 60, headers = headers, timeout = 15000L).document
            ...
            .ifEmpty {
                ...
                if (tmdbId.isNotEmpty()) {
                    ...
                    val imdbId = runCatching {
                        app.get("$TMDBAPI/$type/$tmdbId/external_ids?api_key=$TMDBAPIKEY", timeout = 15000L)
        ...
        if (tmdbIdResolved.isBlank() && imdbUrl.isNotBlank()) {
            ...
            try {
                val findJson = JSONObject(
                    app.get(
                        "$TMDBAPI/find/$imdbIdOnly?api_key=$TMDBAPIKEY&external_source=imdb_id",
                        timeout = 15000L
                    ).text
                )
                ...
                // ignore resolve errors — TMDB enrichment is best-effort
            }
        }
        ...
        val responseData: ResponseDataLocal? = if (tmdbIdResolved.isBlank()) null else runCatching {
            ...
            detailsJson.optJSONObject("credits")?.optJSONArray("cast")?.let { castArr ->
                for (i in 0 until castArr.length()) {
                    ...
                    actorDataList += ActorData(actor = Actor(actorName, profile), roleString = character)
                }
            }
            ...
            if (tvtype == TvType.TvSeries && seasonNumber != null) {
                try {
                    val seasonText = app.get(
                        "$TMDBAPI/tv/$tmdbIdResolved/season/$seasonNumber?api_key=$TMDBAPIKEY",
                        timeout = 15000L
                    ).text
                    ...
                    // ignore season fetch errors — best effort enrichment
                }
            }
        ...
        if (tvtype == TvType.Movie) {
            ...
            doc.select("h3, h4").forEach { element ->
                ...
                val baseLinks = element.select("a[href]").mapNotNull { it.attr("href").takeIf { href -> href.isNotBlank() } }
                ...
                if (isDirectLinkBlock) {
                    baseLinks.forEach { link ->
                        try {
                            val resolvedUrl = getRedirectLinks(link.trim()) ?: link
                            val episodeDoc = app.get(resolvedUrl, timeout = 15000L).document

                            episodeDoc.select("h5 a").forEach { linkElement ->
                                val text = linkElement.text()
                                val epLink = linkElement.attr("href").takeIf { it.isNotBlank() } ?: return@forEach
                                val epNum = Regex("Episode\\s*(\\d+)", RegexOption.IGNORE_CASE).find(text)?.groupValues?.get(1)?.toIntOrNull()

                                if (epNum != null) {
                                    epLinksMap.getOrPut(epNum) { mutableListOf() }.add(epLink)
                                } else {
                                    Log.w(TAG, "Could not parse episode number from: $text")
                                }
                            }
                        } catch (_: Exception) {
                            Log.e(TAG, "Error resolving direct link for URL: $link")
                        }
                    }
                    ...
                    if (element.tagName() == "h4") {
                        ...
                        while (nextElement != null && nextElement.tagName() != "hr") {
                            val siblingLinks = nextElement.select("a[href]").mapNotNull { it.attr("href").takeIf { href -> href.isNotBlank() } }
        ...
        linksList.amap { link ->
            try {
                ...
                when {
                    ...
                    finalLink.contains("hubcloud", ignoreCase = true) -> {
                        HubCloud().getUrl(
                            finalLink,
                            referer = "",
                            subtitleCallback = subtitleCallback,
                ...
                Log.e("HDhub4u-LoadLinks", "Failed to process $link: ${e.message}")
            }
        }
